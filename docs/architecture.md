...
3. Read-only validators measure Korean message join coverage and whether mapped weapon/skill records resolve to mapped public characters. Separate local commands create a non-overwriting private mapping candidate from canonical resolved joins and, for the narrow reviewed protagonist exception, a new revision only after exact structure and Korean-context revalidation. Only an explicitly reviewed mapping converts selected staging rows into versioned `staged` normalized records. Relationship validation remains aggregate-only and does not extend the current version 1 normalized or public schema.
...
6. The public wiki reads only the validated static snapshot bundled with its build. The current file is the explicitly reviewed schema version 1 publication containing 1,683 records; no local database access or private provenance is bundled with it. Reader-facing navigation exposes 1,682 records because the catalog presentation layer withholds the exact `character-pl000b` placeholder row while retaining it in the snapshot.
7. The Sites build compiles only `apps/wiki`, packages its Worker and static assets at the repository root, and rejects artifacts containing known admin, database, or local-path markers. A checked-in schema version 1 publication becomes the current public Sites release only after deployment and a separate verifier confirms that its complete JSON value matches the expected reviewed file. Until then, the latest verified deployment remains a separate prior release. Switching the public origin to a previous saved version remains an explicit rollback operation.
...
The public application currently provides search, category lists, sorting, and record details for the four version 1 collections: characters, weapons, sigils, and skills. Its loader validates the complete snapshot with `publicSnapshotSchema` before rendering. Internal extraction paths, private records, and mining state must never be included in its build. Weapon/skill-to-character relationship coverage is measurable through the private read-only validator. The two private protagonist targets now have an evidence-backed one-to-one mapping revision, an approved two-record diff and complete relationship readiness, but version 1 does not publish or render those links until the versioned schema, migration, backup, baseline and publication gates in [`character-relationship-mapping-policy.md`](character-relationship-mapping-policy.md) are completed.

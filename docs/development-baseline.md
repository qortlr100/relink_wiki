...
| **normalize** | Implemented | Generates a private review candidate from canonical resolved localization joins, maps only the explicitly reviewed JSON into versioned `staged` records, and validates aggregate weapon/skill-to-character relationship readiness without writing data. |
...
The read-only relationship validator checks `weapon.CharaId` and `ability.ReqCharaId1` against `chara.CharId` for rows already selected by the reviewed mapping. It returns aggregate coverage only and blocks relationship publication readiness while any mapped source points to an unmapped or unknown character. The current mapping has complete source references but 69 references across both relation sets still point to two private protagonist variants that are not mapped. Their reviewed public names and one-to-one relationship rule are now fixed by [`character-relationship-mapping-policy.md`](character-relationship-mapping-policy.md); applying that policy to a new private mapping revision and extending the versioned normalized/public schemas remain separate gated work. See [`relationship-validation.md`](relationship-validation.md).

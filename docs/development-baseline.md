...
The read-only relationship validator checks `weapon.CharaId` and `ability.ReqCharaId1` against `chara.CharId` for rows already selected by the reviewed mapping. It returns aggregate coverage only and blocks relationship publication readiness while any mapped source points to an unmapped or unknown character. The current mapping has complete source references but 69 references across both relation sets point to two private characters without an approved public mapping, so the version 1 normalized and public schemas remain unchanged. See [`relationship-validation.md`](relationship-validation.md).

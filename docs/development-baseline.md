...
| **publish**   | Partial     | Local CLI and mining-admin publication controls and the current checked-in schema v1 snapshot are complete; Sites deployment, deployed-JSON verification, future checked-in replacement and rollback remain separate explicit operations.                                                                                       |
| **verify**    | Partial     | The checked-in snapshot passes automated boundaries and local browser checks; the previously deployed snapshot passed complete deployed-JSON comparison. The current checked-in revision still requires deployment verification, and a live rollback transition requires explicit approval and rehearsal.                     |
...
The repository-root `build` keeps the complete workspace build and then copies only the `apps/wiki` Sites Worker, static assets, and hosting manifest into the root `dist/` artifact. The packaging gate parses the hosting manifest, scans every emitted file for known mining-admin, SQLite, and local-path markers, and then statically verifies that the Worker default export owns a `fetch` handler without executing the emitted module. A checked-in schema version 1 snapshot becomes the current public Sites release only after deployment and successful verification. `site:verify` reads only the public JSON endpoint and requires the complete parsed value to match the expected reviewed snapshot before reporting the schema version, content revision and category counts. The 2026-07-17 verification predates the current 2026-07-18 checked-in revision and therefore verifies only the prior deployed release. See [`sites-deployment.md`](sites-deployment.md).

...
The application may call the existing publisher preview in memory when the current baseline remains fully `reviewed`. It displays only the deterministic allowlist content revision and per-category counts. After the same baseline has been finalized as `published`, the panel shows the current recorded publication revision and counts instead.
